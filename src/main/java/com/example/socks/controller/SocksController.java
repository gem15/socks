...
public class SocksController {
    ...
    @ApiResponse(responseCode = "201", description = "Носки оприходованы", content = {
            @Content(mediaType = APPLICATION_JSON_VALUE, schema = @Schema(implementation = Sock.class)) })
    @PostMapping("/income")
    ...
    @ApiResponse(responseCode = "204", description = "Носки отпущены со склада", content = {
            @Content(mediaType = APPLICATION_JSON_VALUE, schema = @Schema(implementation = Sock.class)) })
    @PostMapping("/outcome")
    ...
    @ApiResponse(responseCode = "200", description = "Количество носков", content = {
            @Content(mediaType = APPLICATION_JSON_VALUE, schema = @Schema(implementation = Sock.class)) })
    @GetMapping
     ...
     * )
     * 
     * @ApiResponse(responseCode = "200", description = "Список носков", content =
     * {@Content(mediaType = APPLICATION_JSON_VALUE, schema = @Schema(implementation
     * = Sock.class))})
     * 
     * @GetMapping
     ...
     * @RequestParam("cottonPart") int cottonPart
     * ) {
     * return ResponseEntity.ok(socksService.totalSocks(color, operation,
     * cottonPart));
     * }
     */
